...
_weedZones = ["weed_1"];
_methZones = ["meth_1"];
_grapeZones = ["grapes_1","grapes_2","grapes_3"];
//Create apple zones
{
...
//Create meth zones
{
	...
	_zone setTriggerActivation["CIV","PRESENT",true];
	_zone setTriggerStatements["player in thislist","LIFE_Action_Meth = player addAction['Gather Meth',life_fnc_gatherMeth,'',0,false,false,'','!life_action_inUse'];","player removeAction LIFE_Action_Meth;"];
} foreach _methZones;
//Create grape zones
{
	_zone = createTrigger ["EmptyDetector",(getMarkerPos _x)];
