...
switch (_code) do
{
	...
	{	
		...
		// Civs - > Cops
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget == west) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			[] call life_fnc_restrainAction;
		};
	};
	...
	case 38: 
	{
		...
		if(_shift && playerSide == independent) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F"]) then {
				if(!isNil {vehicle player getVariable "lights"}) then {
					[vehicle player] call life_fnc_MsirenLights;
					_handled = true;
				};
			};
		};
	...
	case 46:
	{
		...
		if(playerSide == independent && vehicle player != player && !life_siren_active && ((driver vehicle player) == player)) then
		{
			[] spawn
			{
				life_Msiren_active = true;
				sleep 4.7;
				life_Msiren_active = false;
			};
			_veh = vehicle player;
			if(isNil {_veh getVariable "siren"}) then {_veh setVariable["siren",false,true];};
			if((_veh getVariable "siren")) then
	...
	case 22:
	{
		if(!_alt && !_ctrlKey) then
		{
			...
			if(_veh in life_vehicles && player distance _veh < 8) then
			{
				...
				_veh say3D "vehicleLock";
				
				// Play unlock sound for all near players.
				_player = _veh nearEntities ["Man", 30];
				{
					if (isPlayer _x) then {
						[[_veh], "life_fnc_playLockVehicle", _x, false] spawn BIS_fnc_MP;
					};
				} forEach _players;
