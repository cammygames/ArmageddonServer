...
if (stop_doubleRobG) exitWith {hint "Someone is already Robbing the gas Station, or failed to capture it recently!";};
stop_doubleRobG = true;
publicVariable "stop_doubleRobG ";

_timer = time + (3 * 60); //Default timer is 3 minutes to rob.
titleText["Robbing Gas Station...","PLAIN"];
[[2,format["$$$ Gassy Gaban is Being Robbed!!! $$$",name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
